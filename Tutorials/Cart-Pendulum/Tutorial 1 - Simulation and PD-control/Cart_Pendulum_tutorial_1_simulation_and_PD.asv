...
(ctrl + shift + enter).
After each section is run, take the time to inspect what happened.
%} 
...
The dataype for the parameters is a specialized class "structor", which can
...
%% We now have s imulation stored in the archive, have a look:
...
C.simulate(duration,init_state.vec, simulator="ode113") % specify "ode113" for fun...
...
%% Forgot the previous simulation? You can still plot it. Lets only plot "x" and "th" this time...
...
C.simulate(50,init_state.vec,...   
   simulator="ode113",...                    % Choose the "ode113" simulator for fun...
...
controller.
In reality, we must measure the state of the system, then compute a contorl
...
th_kp = 1500;
th_kd = 300;
% controller = @(C,t,x) x(1)*x_kp + x(2)*th_kp + x(3)*x_kd + x(4)*th_kd;
...
C.simulate(duration,init_state.vec,...   
   ...
   ode_options       = odeset('RelTol',10^(-7),'AbsTol',10^(-7))); % Let's configue the tolerances just in case. (The default simulator is "ode45", and can be changed)
...
This effect can cause instability, and for NMPC, the control delay is often
very large, since NMPC is computaionally demanding.
%}

close all

% Udpate controller, such that we can time exatcly how long it takes to
...
scale_cd = 10000; % scale control delay by 1e4. Depending on your computer, you may want to adjust this, since the control delay depends on the computer that computes the control signal.
...
% Display previous and new result:
[layout_no_cd,tiles_no_cd] = C.display_simulation("simulation_number",index_without_control_delay,"state",["x","th"],"input","ux");
[layout_cd,tiles_cd]       = C.display_simulation("simulation_number",index_with_control_delay,"state",["x","th"],"input","ux");
