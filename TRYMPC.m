classdef TRYMPC < handle
   ...
   properties
      ...
      parameters (:,1) structor2 = structor2("bulk","first-fields-first") % numerical values for parameters, these values are queried whener the parameters are needed, thus chaning the parameters here, instantly changes the parameters of you system.
      ...
      initial_state (1,1) structor2 = structor2; 
   ...
   methods(Access = public)
      ...
      function def_horizon(C,N,options)
         ...
         C.cas.horizon.decision = structor2("default_mix","TRYMPC_horizon");
         for var_type = C.var_types
            if var_type ~= "param"
         ...
         C.cas.horizon.constraints.equality = structor2("default_mix","separated");
         if C.flag_has_inequality
            C.cas.horizon.constraints.inequality = structor2("default_mix","separated");
         end
         ...
         for k = 1:C.horizon.N
            ...
            if C.integrator.has_aux
               ...
               Arg.aux = structor2.subvec(C.cas.horizon.decision,C.cas.horizon.decision.str.aux.(['stage_',num2str(k-1)]),"separated");
            end
   ...
   methods(Access = private,Hidden)
      ...
      function IRK_builder(C)
         ...
         C.cas.integrator.aux.var = structor2;
         for inc = 1:C.integrator.n_increments
            for samp = 1:samp
         ...
         in_vars = C.cas.var;
         next_state = C.cas.var.state;
         C.cas.integrator.aux.expr = structor2;
         ...
         for inc = 1:C.integrator.n_increments
            ...
            next_state = next_state + (reshape(structor2.subvec(C.cas.integrator.aux.var,C.cas.integrator.aux.var.str.(['f_inc_',num2str(inc)])),C.dim.state,samp)*b) .* (C.cas.integrator.Dt/C.integrator.n_increments);
         end
...
%
      ...
      function collocation_builder(C)
         ...
         Arg.aux = structor2.subvec(C.cas.integrator.aux.var,C.cas.integrator.aux.var.str.increment_1);
         ...
         col_vars = structor2.subvec(C.cas.integrator.aux.var,C.cas.integrator.aux.var.str.increment_1);
         ...
         for inc = 1:C.integrator.n_increments
            ...
            Arg.aux = structor2.subvec(C.cas.integrator.aux.var,C.cas.integrator.aux.var.str.(['increment_',num2str(inc)]));
            Arg.tau = 0;
   ...
   methods(Access = private)
      function def_problem(C,primaldual_type)
         ...
         switch primaldual_type
            case "stacked"
               C.cas.problem.primaldual = structor2("default_mix","separated");
            ...
            case "sorted"
               C.cas.problem.primaldual = structor2("default_mix","TRYMPC_horizon");
      ...
      function find_stats(C)
         ...
         if C.integrator.has_aux
            ...
            C.display.problem.n_aux      = numel(structor2.subvec(C.cas.problem.decision,C.cas.problem.decision.str.aux));
         end
         ...
         if C.flag_has_algeb
            ...
            v = structor2.subvec(C.cas.horizon.constraints.equality,C.cas.horizon.constraints.equality.str.algebraic);
         ...
         if C.flag_stage_constraints_defined && isfield(C.cas.constraints,'equality')
            ...
            v = structor2.subvec(C.cas.horizon.constraints.equality,C.cas.horizon.constraints.equality.str.general);
      ...
      function generate_multipliers(C)
         ...
         C.cas.problem.lambda = structor2("default_mix",C.cas.horizon.constraints.equality.default_mix);
         C.cas.problem.lambda.str = loop_structure(C.cas.horizon.constraints.equality.str,'Lag_mult_lambda');

         % inequality multipliers:
         if C.flag_has_inequality
            C.cas.problem.mu = structor2("default_mix",C.cas.horizon.constraints.inequality.default_mix);
   ...
   methods(Access = private)
      ...
      function pre_initialize_ipopt(C)
         C.internal_mpc.S = structor2(default_mix="TRYMPC_horizon");
...
problems, so maybe I will do this for the reference too at some point.
      ...
      function extract_solution_ipopt(C)
         ...
         C.internal_mpc.solution.dual_eq = structor2.subvec(S_num,S_num.str.equality);
         if C.flag_has_inequality
            C.internal_mpc.solution.dual_in = structor2.subvec(S_num,S_num.str.inequality);
         end
      ...
      function call_sqp(C)
         ...
         C.internal_mpc.solution.dual_eq = structor2.subvec(C.cas.problem.primaldual,primaldual.str.lambda);
         if C.flag_has_inequality
            C.internal_mpc.solution.dual_in = structor2.subvec(C.cas.problem.primaldual,primaldual.str.mu);
         end
...
%}
   ...
   methods
      ...
      function initial_guess = sim2guess(C,sim_number)
         arguments
            C
            sim_number (1,1) double {mustBeInteger,mustBePositive} = length(C.archive.simulations);
         end
         time = C.archive.simulations{sim_number}.sim.time;
         initial_guess = C.cas.horizon.decision.zeros;
         initial_guess.str.state = interp1(time,C.archive.simulations{sim_number}.sim.state',linspace(time(1),time(2),C.horizon.N+1))';
         
         if isfield(C.cas.horizon.decision,'aux')
            names = fieldnames(C.cas.horizon.decision.str.aux);
            for i = 1:length(names)
               initial_guess.str.aux.(names{i}) = loop(C.cas.horizon.decision.str.aux.(names{i}));
            end
         end
         ...
         function out = loop(S)
            if isstruct(S)
               for name = string(fieldnames(S))'
                  out.(name) = loop(S.(name));
               end
            else
               out = initial_guess.str.state(:,i);
            end
         end
      end
   
      function initial_guess = opt2guess(C,opt_number)
         arguments
            C
            opt_number (1,1) double {mustBeInteger,mustBePositive} = length(C.archive.optimizations);
         end
         time = C.archive.optimizations{opt_number}.time;
         initial_guess.primal = C.cas.horizon.decision.zeros;
         new_time = linspace(time(1),time(2),C.horizon.N+1);
         initial_guess.primal.str.state = interp1(time,C.archive.optimizations{opt_number}.decision.str.state',new_time)';

         if isfield(C.cas.horizon.decision,'aux')
            names = fieldnames(C.cas.horizon.decision.str.aux);
            for i = 1:length(names)
               initial_guess.primal.str.aux.(names{i}) = loop_primal(C.cas.horizon.decision.str.aux.(names{i}));
            end
         end

         %%% Lagrange Multipliers: (obs this is not a good way to get an initial guess for duals)

         dual = C.archive.optimizations{opt_number}.dual_eq;
         L_dual = length(dual);
         initial_guess.dual_eq = interp1((1:L_dual)/L_dual,dual,(1:C.cas.horizon.constraints.equality.len)/L_dual);
         
         dual = C.archive.optimizations{opt_number}.dual_in;
         L_dual = length(dual);
         initial_guess.dual_in = interp1((1:L_dual)/L_dual,dual,(1:C.cas.horizon.constraints.inequality.len)/L_dual);

         %%% Internal functions
         function out = loop_primal(S)
            if isstruct(S)
               for name = string(fieldnames(S))'
                  out.(name) = loop(S.(name));
               end
            else
               out = initial_guess.primal.str.state(:,i);
            end
         end
      end
   end
   ...
   methods
      ...
      function [tiles,Layout] = display_simulation(C,options)
         arguments
            ...
            options.reference (1,:) string {mustBeMember(options.reference,["off","state","algeb","input","only"])} = "off"; % whether or not to plot the reference
         end
         ...
         for i = 1:L
            for type = Types
               ...
               for name = names
                  ...
                  % Time:
                  time_in_seconds = data(options.display_number(i)).time;
                  time = time_scaler(time_in_seconds);
                  ...
                  if isfield(options,'colors')
                     if isa(options.colors,"string")
                        color = GetColorCode(options.colors(min(i,end)));
                  ...
                  if ismember(type,options.reference)
                     ref = C.reference.(type)(time_in_seconds); % evaluate the reference signals at times given in SECONDS!
                     ref = ref(C.names.ind.(type).(name),:);
                     plot(tile,...
                        time,...
                        ref,...
                        Color=[1 1 1]*0.5,...
                  ...
                  if ~ismember("only",options.reference)
                     plot(tile,...
                        time,...
                        variable,...
                     ...
                     if display_type == "simulation" && ~isinf(C.archive.simulations{options.display_number(i)}.sampling_time) && options.mark_samples && ismember(type,["state","algeb"])
                        ...
                        samp_time = time_scaler([C.archive.simulations{options.display_number(i)}.stage(1:inc:end).time]);
