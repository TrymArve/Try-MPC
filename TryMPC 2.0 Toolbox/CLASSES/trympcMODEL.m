classdef trympcMODEL
   ...
   properties(SetAccess=immutable)
      ...
      % expressions
      dynamics      %    x_dot = f(x,z,u,p)
      algebraics    %        0 = g(x,z,u,p)
      output        %   y = output(x,z,u,p)
   ...
   properties(Hidden,SetAccess=immutable)
      ...
      variable_types = ["state", "algeb", "input"];
      variable_types_par = ["state", "algeb", "input", "param"];
      expression_types
   ...
   methods
      function C = trympcMODEL(model_name,names,model)
         ...
         elseif isfield(model,'algebraics')
            C.model_type = "DAE";
         else
            C.model_type = "ODE";
         end
         ...
         %%% Create dynamics / algebraics / ouput
         for expr_type = C.expression_types


            try
               expr = model.(expr_type)(C.state,C.algeb,C.input,C.param);
            catch ME
         ...
         if C.model_type == "DAE"
            C.algebraics_fast = casadi.Function('F_algebraics_fast',{args.state,args.algeb,args.input,args.param},{C.algebraics.call(args).out});
         end
